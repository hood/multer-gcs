...
export default class MulterGCS implements multer.StorageEngine {
	private gcobj: Storage;
	private gcsBucket: Bucket;
	private options: ConfigurationObject & {
		acl?: string;
		bucket?: string;
	...
	constructor(
		...
		if (!opts.credentials)
			throw new Error(
				"You have to specify credentials for Google Cloud Storage to work."
			);
...
export function storageEngine(
	opts?: ConfigurationObject & { filename?: any; bucket?: string }
) {
